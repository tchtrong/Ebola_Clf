...
class Corpus(object):
    ...
    def build_vocabulary(self):
        ...
        try:
            ...
            for i in lines:
                self.motifs.append(i)
        finally:
            file.close()

        discrete_set = set()
        for motif in self.motifs:
            discrete_set.add(motif)
    ...
    def plsa(self, number_of_topics, max_iter):
        ...
        for d_index, doc in enumerate(self.documents):
            ...
            for w_index, word in enumerate(self.vocabulary):
                count = 0
                iterator = re.finditer(word, doc)
                for match in iterator:
                    count += 1
                term_count[w_index] = count
        ...
        self.topic_prob = np.zeros([number_of_documents, len(self.vocabulary), number_of_topics],
            ...
            for d_index, document in enumerate(self.documents):
                for w_index in range(vocabulary_size):
                    ...
                    if sum(prob) == 0.0:
                        ...
                        # print("self.document_topic_prob"  + "["  + str(d_index) + ":] = " + str(self.document_topic_prob[d_index, :]))
                        # print("self.topic_word_prob[:" +  str(w_index) + "] = " + str(self.topic_word_prob[:, w_index]))
                        # print("topic_prob[" + str(d_index) + "]"+ "["+str(w_index) + "]="  + str(prob))
                        exit(0)
                    else:
                        normalize(prob)
